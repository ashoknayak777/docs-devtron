...
| **Authentication Type** | Select one of the authentication types:<br/><br/>- **EC2 IAM Role**: Authenticate with workernode IAM role and attach the ECR policy (`AmazonEC2ContainerRegistryFullAccess`) to the cluster worker nodes IAM role of your Kubernetes cluster.<br/><br/>- **User Auth**: It is a key-based authentication, attach the ECR policy (`AmazonEC2ContainerRegistryFullAccess`) to the [IAM user](https://docs.aws.amazon.com/AmazonECR/latest/userguide/get-set-up-for-amazon-ecr.html).<br/> &nbsp;&nbsp;&nbsp;&nbsp;- `Access key ID`: Your AWS access key<br/> &nbsp;&nbsp;&nbsp;&nbsp;- `Secret access key`: Your AWS secret access key ID |
